@extends('layout')
@section('item', 'link_6')
@section('name_head', 'Настройки → Дополнительные настройки')
@section('content')
            ...
            <div class="mb-2 row">
                ...
                <div class="col-sm-7">
                    <select name="Saleschannel" class="form-select text-black ">
                        ...
                        @else
                            <option value="{{$Saleschannel}}" selected> {{$Saleschannel}} </option>
                        ...
                        @foreach($Body_saleschannel as $Body_saleschannel_item)
                            @if ($Saleschannel != $Body_saleschannel_item->name)
                                <option
                                    value="{{ $Body_saleschannel_item->name }}"> {{ ($Body_saleschannel_item->name) }} </option>
                            @endif
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="Project" class="form-select text-black ">
                        ...
                        @foreach($Body_project as $Body_project_item)
                            @if ($Project != $Body_project_item->name)
                                <option
                                    value="{{ $Body_project_item->name}}"> {{ ($Body_project_item->name) }} </option>
                            @endif
                        @endforeach
                    </select>
                </div>
